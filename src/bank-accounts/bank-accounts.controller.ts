...
export class BankAccountsController {
  ...
  create(@Body() createBankAccountDto: CreateBankAccountDto, @CurrentUser() user: User): Promise<BankAccount> {
  ...
  findOne(@Param('id') id: number, @CurrentUser() user: User): Promise<BankAccount> {
    return this.bankAccountsService.findOne(id, user.id);
  }

  @Patch(':id')
  @ApiResponse({ status: 200, description: 'Update a bank account.' })
  update(@Param('id') id: number, @Body() updateBankAccountDto: UpdateBankAccountDto, @CurrentUser() user: User): Promise<BankAccount> {
