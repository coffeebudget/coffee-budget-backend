...
describe('PendingDuplicatesService', () => {
  ...
  describe('findPendingDuplicates', () => {
    it('should return pending duplicates for a user', async () => {
      const mockDuplicates = [
        { id: 1, resolved: false, existingTransaction: { id: 1, description: 'Existing Transaction', amount: 100 } }
      ];
      (pendingDuplicatesRepository.find as jest.Mock).mockResolvedValue(mockDuplicates);
  ...
  describe('update', () => {
    it('should update a pending duplicate', async () => {
      ...
      (pendingDuplicatesRepository.findOne as jest.Mock).mockResolvedValue(mockDuplicate);
      (pendingDuplicatesRepository.save as jest.Mock).mockImplementation(entity => Promise.resolve(entity));
    ...
    it('should throw NotFoundException when duplicate not found', async () => {
      (pendingDuplicatesRepository.findOne as jest.Mock).mockResolvedValue(null);

      await expect(service.update(999, { resolved: true }, 1))
  ...
  describe('createPendingDuplicate', () => {
    it('should create a pending duplicate with source information', async () => {
      const existingTransaction = { id: 1, description: 'Existing Transaction', amount: 100 };
      const newTransactionData = { description: 'New Transaction', amount: 100, categoryId: 1 };
      const userId = 1;
      ...
      pendingDuplicate.existingTransactionData = JSON.stringify(existingTransaction);
      ...
      (pendingDuplicatesRepository.save as jest.Mock).mockResolvedValue(pendingDuplicate);
  ...
  describe('resolvePendingDuplicate', () => {
    it('should resolve a pending duplicate with MAINTAIN_BOTH choice', async () => {
      ...
      (pendingDuplicatesRepository.findOne as jest.Mock).mockResolvedValue(pendingDuplicate);
      (transactionOperationsService.handleDuplicateResolution as jest.Mock).mockResolvedValue({
      ...
      expect(transactionOperationsService.handleDuplicateResolution).toHaveBeenCalledWith(
    ...
    it('should throw NotFoundException if pending duplicate is not found', async () => {
      ...
      await expect(service.resolvePendingDuplicate(
        1,
        1,
        DuplicateTransactionChoice.MAINTAIN_BOTH
      )).rejects.toThrow(NotFoundException);
    ...
    it('should handle KEEP_EXISTING choice even if resolution throws an error', async () => {
      const mockPendingDuplicate = {
        ...
        newTransactionData: { description: 'New Transaction', amount: 100, categoryId: 1 },
        resolved: false,
        user: { id: 1 }
      };

      (pendingDuplicatesRepository.findOne as jest.Mock).mockResolvedValue(mockPendingDuplicate);
      (transactionOperationsService.handleDuplicateResolution as jest.Mock).mockRejectedValue(new Error('Some error'));
      (pendingDuplicatesRepository.save as jest.Mock).mockImplementation(entity => Promise.resolve(entity));
