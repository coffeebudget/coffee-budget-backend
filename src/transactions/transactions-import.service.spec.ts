...
describe('TransactionsService - Import', () => {
  ...
  beforeEach(async () => {
        ...
        },
        {
          ...
          useValue: {
            createAutomatedTransaction: jest.fn(
              (data, userId) => ({ id: 999, ...data, user: { id: userId } })
            ),
          },
        },
      ],
    ...
    tagRepository = module.get<Repository<Tag>>(
    ...
    jest.spyOn(transactionRepository, 'save').mockImplementation(entity => Promise.resolve({ id: 999, ...entity } as Transaction));
    jest.spyOn(transactionRepository, 'create').mockImplementation(entity => entity as Transaction);
    
    jest.spyOn(bankAccountRepository, 'findOne').mockResolvedValue({ id: 123, name: 'Test Account' } as BankAccount);
    jest.spyOn(creditCardRepository, 'findOne').mockResolvedValue({ id: 456, name: 'Test Card', billingDay: 15 } as CreditCard);
  ...
  describe('importTransactions', () => {
    it('should throw BadRequestException if no CSV data or column mappings are provided for generic import', async () => {
      await expect(service.importTransactions({ 
    ...
    it('should use the appropriate bank parser for a supported bank format', async () => {
      ...
      expect(recurringPatternDetectorService.detectAllRecurringPatterns).toHaveBeenCalled();
    });
    ...
    it('should handle generic CSV import correctly', async () => {
      ...
      require('csv-parse/sync').parse = jest.fn().mockReturnValue([
        { description: 'Test Transaction', amount: '100.50', date: '2023-02-01' }
      ]);
      ...
      (categoriesService.suggestCategoryForDescription as jest.Mock).mockResolvedValue({
        id: 1, name: 'Suggested Category'
      });
      ...
      try {
        const result = await service.importTransactions(mockImportDto as any, 1);
        
        expect(result).toBeDefined();
        expect(transactionOperationsService.createAutomatedTransaction).toHaveBeenCalled();
      } finally {
    ...
    it('should calculate billing date for credit card transactions', async () => {
      ...
      const createAutoTxSpy = jest.spyOn(transactionOperationsService, 'createAutomatedTransaction')
      ...
      const creditCardTxCall = calls.find(call => call[0].creditCard?.id === 456);
      ...
      if (creditCardTxCall) {
        ...
        expect(txData.billingDate?.getFullYear()).toBe(expectedBillingDate.getFullYear());
        expect(txData.billingDate?.getMonth()).toBe(expectedBillingDate.getMonth());
        expect(txData.billingDate?.getDate()).toBe(expectedBillingDate.getDate());
      }
    });
    ...
    it('should calculate billing date for credit card transactions if not provided', async () => {
      ...
      const calls = (transactionOperationsService.createAutomatedTransaction as jest.Mock).mock.calls;
    ...
    it('should preserve existing billing date if provided in parsed transaction', async () => {
      ...
      const createAutoTxCallArgs = (transactionOperationsService.createAutomatedTransaction as jest.Mock).mock.calls[0][0];
      ...
      expect(createAutoTxCallArgs.description).toBe('Credit Card Purchase with Custom Billing');
    });
  });
});
