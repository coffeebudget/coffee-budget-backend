    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        if (
          rows[i] && 
          rows[i][0] === 'Data' && 
          (rows[i].includes('Entrate') || rows[i].includes('Uscite'))
        ) {
          headerRowIndex = i;
          break;
        }
      ...
      if (headerRowIndex === -1) {
        ...
        for (let i = 0; i < rows.length; i++) {
          if (rows[i] && rows[i][0] === 'Data contabile') {
            headerRowIndex = i;
            break;
          }
        }
      ...
      if (dateIndex === -1) {
        throw new BadRequestException('Could not find date column in BNL Excel file');
      }
      
      if (descriptionIndex === -1 && detailedDescriptionIndex === -1) {
        throw new BadRequestException('Could not find description column in BNL Excel file');
      }
      ...
      for (let i = headerRowIndex + 1; i < rows.length; i++) {
        ...
        if (!row[dateIndex] || row.length < Math.max(dateIndex, entrateIndex, usciteIndex) + 1) {
        ...
        try {
          ...
          if (entrateIndex !== -1 && usciteIndex !== -1) {
            ...
            const entrateValue = row[entrateIndex] ? this.parseAmount(row[entrateIndex] as string) : 0;
            const usciteValue = row[usciteIndex] ? this.parseAmount(row[usciteIndex] as string) : 0;
