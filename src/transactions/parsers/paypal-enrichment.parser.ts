...
export class PayPalEnrichmentParser {
  ...
  async parseAndEnrich(data: string, options: {
    userId: number;
    ...
    try {
      ...
      this.logger.debug(`Parsed ${paypalTransactions.length} PayPal transactions`);

      // Filter valid transactions (completed, with name, not generic funding)
      const validTransactions = this.filterValidTransactions(paypalTransactions);
      
      this.logger.debug(`Found ${validTransactions.length} valid PayPal transactions for enrichment`);
      ...
      throw new BadRequestException(`Failed to parse PayPal data: ${error.message}`);
    }
  }
  ...
  private async parsePayPalCsv(csvData: string): Promise<PayPalTransaction[]> {
    try {
      ...
      const expectedHeaders = ['Data', 'Nome', 'Tipo', 'Stato', 'Valuta', 'Importo'];
      ...
      return records.map(record => {
        try {
          ...
          let amount = parseLocalizedAmount(rawAmount);
          ...
          this.logger.warn(`Skipping record due to parsing error: ${error.message}`);
          return null;
        }
      }).filter(Boolean);
    } catch (error) {
      ...
      throw new BadRequestException(`Failed to parse PayPal CSV: ${error.message}`);
    }
  }

  private filterValidTransactions(transactions: PayPalTransaction[]): PayPalTransaction[] {
    ...
    for (const paypalTx of paypalTransactions) {
      try {
        ...
        if (matchingTransactions.length > 0) {
          ...
          const enrichedDescription = this.createEnrichedDescription(match.description, paypalTx.name);
          
          // Update the transaction
          match.description = enrichedDescription;
          
          // Log the enrichment details
          this.logger.debug(`Enriching transaction: "${originalDescription}" -> "${enrichedDescription}"`);
        ...
        this.logger.warn(`Error processing PayPal transaction: ${error.message}`);
        unmatched.push(paypalTx);
      }
    }
   ...
   */
  private createEnrichedDescription(originalDesc: string, paypalMerchant: string): string {
    if (!originalDesc) {
