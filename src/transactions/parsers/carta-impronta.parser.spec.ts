...
describe('CartaImprontaParser', () => {
  ...
  describe('parseFile', () => {
    ...
    it('should create transactions with correct data', async () => {
      ...
      const transactions = await parser.parseFile('test-data', {
        userId: 1,
        creditCardId: 123,
      });
    ...
    it('should handle empty data', async () => {
      await expect(parser.parseFile('', { userId: 1 })).rejects.toThrow(
        'Missing CartaImpronta HTML content',
      );
    });

    it('should handle invalid HTML data', async () => {
      await expect(
        parser.parseFile('<div>No table here</div>', { userId: 1 }),
      ).rejects.toThrow('Invalid CartaImpronta HTML format');
    });
  });
});
