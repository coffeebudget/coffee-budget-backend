...
export class CartaImprontaParser extends BaseParser {
  async parseFile(
    ...
    try {
      ...
      this.logger.debug(
        `[CARTA_IMPRONTA IMPORT] HTML loaded successfully, length: ${htmlContent.length}`,
      );
      ...
      if (!table.length) {
        this.logger.debug(
          '[CARTA_IMPRONTA IMPORT] Table not found by ID, trying alternative selectors',
        );
      ...
      if (!table.length) {
        ...
        this.logger.warn(
          `[CARTA_IMPRONTA IMPORT] No suitable table found (total tables: ${tableCount})`,
        );
        throw new BadRequestException(
          'Invalid CartaImpronta HTML format: table not found',
        );
      }
      ...
      table.find('tbody tr').each((index, row) => {
        ...
        if (cells.length < 5) {
          this.logger.warn(
            `[CARTA_IMPRONTA IMPORT] Skipping row ${index + 1}: Not enough cells (found ${cells.length}, expected at least 5)`,
          );
        ...
        if (!dateStr || !amountEuroStr || !description) {
          this.logger.warn(
            `[CARTA_IMPRONTA IMPORT] Skipping row ${index + 1}: Missing required data`,
          );
          return; // continue to next row
        }
        ...
        try {
          ...
          this.logger.warn(
            `[CARTA_IMPRONTA IMPORT] Skipping row ${index + 1}: ${error.message}`,
          );
        }
      });

      this.logger.debug(
        `[CARTA_IMPRONTA IMPORT] Found ${transactions.length} transactions`,
      );

      if (transactions.length === 0) {
        this.logger.warn(
          '[CARTA_IMPRONTA IMPORT] No transactions found in the table',
        );
      }
      ...
      this.logger.error(
        `[CARTA_IMPRONTA IMPORT] Error parsing file: ${error.message}`,
      );
      throw new BadRequestException(
        `Failed to parse CartaImpronta file: ${error.message}`,
      );
    }
  }
}
